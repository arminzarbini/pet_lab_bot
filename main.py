...
@bot.message_handler(func=lambda message: message.text==buttons_admin['home'])
...
def create_test_group_handler(message):
    ...
    if cid in admins:
        markup = send_home()
        bot.send_message(cid, text_admin['test_group_input'], reply_markup=markup)
        user_steps[cid] = 1
    else :
        unknown_message()
...
def group_test_handler(message):
    ...
    if cid in admins:
        ...
        for group in test_group:
            exist_test_group_name.append(group['name'])
        if name in exist_test_group_name:
            bot.send_message(cid, text_admin['test_group_unique'])
        elif len(name) > 45:
    ...
    if cid in admins:
        ...
        if len(item_test_check) != len(item_title):
            ...
            for key,value in item_check.items():  
                if key == 'parameter':
                    parameter = value
                    exist_test_parameter = show_test_parameter()
                    exist_parameter = list()
                    for param in exist_test_parameter:
                            exist_parameter.append(param['parameter'])
                    if parameter == "":
                        bot.send_message(cid, text_admin['parameter_test_null'])
                    else :
                        if parameter in exist_parameter:
...
def username_edit_handler(message):
    ...
    if cid not in admins:
        ...
        exist_user_username = show_user_username()
        exist_username = list()
        for user in exist_user_username:
            exist_username.append(user['username'])
        english_flag = True
